...
from scripts.model import UNetResidual, UNetResidual_Ext2
...
def combine128(output_list, dim, position):
    ...
    for ind in range(len(output_list)):
        ...
        result[top:bot, left:right, :] = output[33:33 +(bot-top), 33:33 + (right-left), :]
...
def run_infer(sess, image_holder, output, patch):
    ...
    patch_res_list = np.array(patch_res_list[0])
    return patch_res_list
...
def infer(sess, image_holder, output, img_link):
    ...
    for patch in patch_list:
        patch_res = run_infer(sess, image_holder, output, patch)
        output_patch_list.append(patch_res)
...
def main():
    ...
    # Image holder
    image_holder = tf.placeholder(tf.float32, shape=(1, h, w, 3))

    # Create network
    net = UNetResidual_Ext2({'data': image_holder}, is_training=False, num_classes=args.num_classes)
